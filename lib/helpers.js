...
const ADMIN_PROVIDER = (process.env.ADMIN_PROVIDER || 'google').toLowerCase();
function linkOwnerAccount(sb) {
  const adminEmail = String(process.env.ADMIN_EMAIL || '').toLowerCase();
  if (!adminEmail || sb.email !== adminEmail) return null;
  if (!sb.emailVerified) return null;
  if (!Array.isArray(sb.providers) || !sb.providers.includes(ADMIN_PROVIDER)) return null;
...
function sweepRateLimits() {
  ...
  for (const map of [authAttempts, writeAttempts, geocodeAttempts, apiAttempts]) {
    for (const [k, arr] of map) {
      if (!arr.length || now - arr[arr.length - 1] >= RATE_LIMIT_SWEEP_WINDOW_MS) map.delete(k);
    }
  }
}
