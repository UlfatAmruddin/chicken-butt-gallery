...
function authUser(req) {
  ...
  // Legacy string-format sessions predate the TTL + per-user cap. Upgrading them
  // (as before) re-stamped a fresh 30-day expiry on every use, so such a token
  // never expired and dodged the cap. Force re-auth instead - all live sessions are
  // then TTL-bounded objects.
