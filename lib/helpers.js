...
function clientIp(req) {
  // Behind a trusted reverse proxy (Vercel and most PaaS), key rate limits off the
  // client IP the PROXY reports, never a client-supplied one. Prefer the platform
  // header (x-real-ip is the real client on Vercel), then the RIGHTMOST
  // X-Forwarded-For hop the proxy appended - never the leftmost, which the client
  // can forge to rotate rate-limit keys. Only trust these when TRUST_PROXY is set;
  // otherwise ignore them entirely (direct-bind local dev, where XFF is spoofable).
...
function tooManyAuthAttempts(req, username) {
  ...
  // two independent limiters: per source-IP+user, and per-username across all
  // IPs, so one account can't be brute-forced by rotating source addresses.
  const ipOver = bumpWindow(authAttempts, `${clientIp(req)}:${uname}`, AUTH_WINDOW_MS, AUTH_MAX_ATTEMPTS);
