...
// Auto-link the site owner (email === ADMIN_EMAIL) to the reserved admin app-user,
// so signing in with Google keeps ownership of existing communities with no username
// step. Nobody else can claim it - it is gated purely on the verified email.
...
function sweepRateLimits() {
  const now = Date.now();
  for (const map of [authAttempts, writeAttempts, geocodeAttempts]) {
