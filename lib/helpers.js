...
const supaAuth = require('./supabase-auth');
const {
...
const AUDIT_MAX_PER_COMMUNITY = 500;   // retained audit events per community
...
// Session tokens are stored HASHED (sha256) at rest, so a leaked sessions store or
// backup is not a pile of ready-to-use bearer credentials. The raw token is handed to
// the client once at login; only its hash is ever persisted or looked up.
function hashToken(token) {
  return crypto.createHash('sha256').update(String(token)).digest('hex');
}
/* ---------------- identity (Supabase Auth / Google) ----------------
   Login is done by Supabase Auth on the client; each request carries a Supabase
   access token. resolveAuth() validates it once per request (see handleApi) and
   the result is hung on req._auth = { supabase:{id,email}, user:<appUser|null> }.
   A null user is a valid Google login that has not chosen a username yet.
   authUser() stays synchronous so the many existing call sites don't each await. */
async function resolveAuth(req) {
  const token = (req.headers.authorization || '').replace(/^Bearer\s+/i, '');
  if (!token) return null;
  const sb = await supaAuth.validateAccessToken(token);
  // Could not verify with Supabase: propagate so the caller answers 503, never 401.
  // A false "not logged in" makes every client wipe its session (mass logout).
  if (sb === supaAuth.UPSTREAM) return supaAuth.UPSTREAM;
  if (!sb) return null;
  let user = findUserBySupabaseId(sb.id);
  if (!user) user = linkOwnerAccount(sb);   // owner keeps the pre-existing admin account
  return { supabase: sb, user: user || null };
}
function authUser(req) {
  const a = req._auth;
  return a && a.user ? { user: a.user, supabase: a.supabase } : null;
}
function findUserBySupabaseId(id) {
  if (!id) return null;
  for (const name of Object.keys(users)) if (users[name].supabaseId === id) return users[name];
  return null;
}
// Auto-link the site owner to the reserved admin app-user, so signing in with Google
// keeps ownership of existing communities with no username step.
//
// This is the single highest-privilege grant in the app, so an email STRING is not
// enough on its own: anyone able to mint a Supabase token carrying that address (e.g.
// an email/password signup on a project where confirmation is off) would otherwise
// capture full site admin. Require, in addition to the address matching:
//   - Supabase has CONFIRMED the address (email_confirmed_at), and
//   - the account authenticated through the trusted OAuth provider (Google),
// both of which only hold for a real Google sign-in. Fails closed.
const ADMIN_PROVIDER = (process.env.ADMIN_PROVIDER || 'google').trim().toLowerCase();
function linkOwnerAccount(sb) {
  const adminEmail = String(process.env.ADMIN_EMAIL || '').trim().toLowerCase();
  if (!adminEmail || sb.email !== adminEmail) return null;
  // Bind to the SPECIFIC identity that owns the address, not to account-level facts.
  // A union ("some identity on this account is google") would be satisfied by an
  // account that merely has a Google identity attached alongside another sign-in
  // method, so it never proved the Google identity owns ADMIN_EMAIL.
  const owner = (sb.identities || []).find(i => i.provider === ADMIN_PROVIDER && i.email === adminEmail);
  if (!owner || !(owner.emailVerified || sb.emailVerified)) {
    // make the refusal diagnosable - otherwise the owner silently lands in onboarding
    console.warn(`[auth] refusing owner link for ${adminEmail}: no verified ${ADMIN_PROVIDER} identity for that address`);
    return null;
  }
  for (const name of ADMIN_USERNAMES) {
    const u = users[name];
    if (!u) continue;
    if (u.supabaseId === sb.id) return u;
    if (!u.supabaseId) { u.supabaseId = sb.id; u.email = sb.email; saveJSON('users.json', users); return u; }
  }
  return null;
}
/* Create the app profile for a freshly-authenticated Google user who has picked a
   username. No password/salt any more - identity is the linked Supabase account. */
function createAppUser(username, sb, displayName) {
  users[username] = {
    username,
    displayName: clean(displayName, 40) || username,
    supabaseId: sb.id,
    email: sb.email,
...
function createSession(username) {
  ...
  sessions[hashToken(token)] = { username, created: now, expires: now + SESSION_TTL_MS };
  saveJSON('sessions.json', sessions);
  return token;   // raw token handed to the client once; only its hash is stored
}
function clientIp(req) {
  // Behind a trusted reverse proxy, derive the client IP ONLY from headers the proxy
  // itself sets - never a raw client-supplied header - and validate every candidate
  // as a real IP so an attacker can't seed unbounded rate-limit keys with arbitrary
  // strings. Only consult forwarding headers when TRUST_PROXY is set; otherwise use
  // the socket (direct-bind local dev, where forwarding headers are spoofable).
  if (process.env.TRUST_PROXY === '1') {
    // Cloudflare's CF-Connecting-IP is only trustworthy when a Cloudflare edge
    // actually fronts the app (it overwrites any client-sent value there). On a host
    // WITHOUT Cloudflare (e.g. Vercel) it is raw client input, so honoring it
    // unconditionally let an attacker send a fresh CF-Connecting-IP per request and
    // rotate every per-IP rate-limit key at will. Trust it only behind an explicit
    // TRUST_CF_IP opt-in; otherwise use the RIGHTMOST X-Forwarded-For hop, which the
    // platform's own proxy appends (left hops are client-forgeable). `x-real-ip` is
    // never trusted (pure client input on some stacks).
...
// Hard ceiling on distinct keys per rate-limit map. tooManyAuthAttempts seeds a
// key per (ip, username) and per username BEFORE any existence check, so a flood of
// unique random usernames would otherwise grow these maps without bound until the
// process OOMs. A periodic sweep (below) drops aged-out keys in steady state; this
// cap is the backstop against a burst that outruns the sweep.
const RATE_LIMIT_MAX_KEYS = 50000;
function evictOldest(map, n) {
  let i = 0;
  for (const k of map.keys()) { map.delete(k); if (++i >= n) break; }   // Map preserves insertion order
}
// Fixed-cost counter per key, NOT a growing array of timestamps. The array version
// allocated and re-scanned every hit still in the window on EVERY request, and kept
// pushing even while the key was already blocked - so the harder a source flooded,
// the more CPU and memory each rejection cost it. The counter stops climbing once
// the cap is passed, making a throttled attacker cheap to refuse.
function bumpWindow(map, key, windowMs, cap) {
  const now = Date.now();
  let e = map.get(key);
  if (!e || now - e.start >= windowMs) { e = { n: 0, start: now, last: now }; map.set(key, e); }
  e.last = now;
  if (e.n <= cap) e.n += 1;   // clamp at cap+1: over-limit hits add no information
  if (map.size > RATE_LIMIT_MAX_KEYS) evictOldest(map, map.size - RATE_LIMIT_MAX_KEYS);
  return e.n > cap;
}
...
// Blanket per-IP ceiling on ALL /api/* traffic, reads included. tooManyWrites only
// covers mutating methods, so GET floods were previously unbounded - and every request
// runs token validation, which can reach out to Supabase. Set generously so normal
// browsing (including several people sharing one NAT) never trips it.
const apiAttempts = new Map();
const API_WINDOW_MS = 60 * 1000;
const API_MAX_PER_MIN = 600;
function tooManyApiRequests(req) {
  return bumpWindow(apiAttempts, clientIp(req), API_WINDOW_MS, API_MAX_PER_MIN);
}
// Dedicated, much stricter limiter for the geocode proxy. Each distinct query
// misses the cache and forces an outbound Nominatim request from the server IP; the
// generic write throttle (120/min) is far above OSM's ~1 req/sec policy, so a member
// could grind it and get the shared IP banned. Keep geocoding well under 1 req/sec.
const geocodeAttempts = new Map();
const GEOCODE_WINDOW_MS = 60 * 1000;
const GEOCODE_MAX_PER_MIN = 15;
function tooManyGeocodes(req, username) {
  const ip = clientIp(req);
  const u = clean(username, 20).toLowerCase();
  return bumpWindow(geocodeAttempts, `${ip}:${u}`, GEOCODE_WINDOW_MS, GEOCODE_MAX_PER_MIN);
}
// Steady-state sweep: drop keys whose newest hit has aged out of the longest window,
// so the rate-limit maps don't accumulate one dead entry per attempted username/IP
// forever. Unref'd so it never keeps the process alive on its own (and it is a
// harmless no-op on frozen serverless instances).
const RATE_LIMIT_SWEEP_WINDOW_MS = Math.max(AUTH_WINDOW_MS, WRITE_WINDOW_MS, GEOCODE_WINDOW_MS);
function sweepRateLimits() {
  const now = Date.now();
  for (const map of [authAttempts, writeAttempts, geocodeAttempts, apiAttempts]) {
    for (const [k, e] of map) {
      if (!e || now - e.last >= RATE_LIMIT_SWEEP_WINDOW_MS) map.delete(k);
    }
  }
}
const rateLimitSweepTimer = setInterval(sweepRateLimits, 5 * 60 * 1000);
...
function canChangeMember(actorRole, targetRole, nextRole, actorUsername, targetUsername) {
  // The next role must always be a known role - even for a global admin - so a bad
  // payload (e.g. role:"superuser") can't persist a garbage string into the roster.
  if (nextRole !== 'owner' && nextRole !== 'admin' && nextRole !== 'member') return false;
...
function requireAuth(req, res) {
  const a = req._auth;
  if (!a) { send(res, 401, { error: 'Not logged in.' }); return null; }
  // valid Google login, but no app profile yet -> the client should run onboarding
  if (!a.user) { send(res, 409, { error: 'Finish setting up your account.', needsOnboarding: true }); return null; }
  return { user: a.user, supabase: a.supabase };
}
...
function publicProfile(u, communityId = '', photoCount = null) {
  return {
    username: u.username,
    ...
    cover: u.cover || '',
    joined: u.joined,
    // callers rendering many profiles at once (memberList) can pass a precomputed
    // count to avoid a full posts scan per profile; otherwise compute it here.
    photoCount: photoCount != null ? photoCount : userPhotoCount(u.username, communityId),
...
function saveDataUrlImage(dataUrl, relDir, basename, maxBytes = 12 * 1024 * 1024) {
  // same single validator as the Supabase path (format allow-list + size + magic
  // bytes + decompression-bomb dimensions) so the two backends never drift.
  const v = supa.decodeAndValidateImageDataUrl(dataUrl, maxBytes);
  if (v.error) return { error: v.error };
  const { buf, ext } = v;
  const safe = String(basename).replace(/[^a-z0-9_-]/gi, '') || 'img';
  // 128 bits of randomness, matching the Supabase Storage path: assets/ is served to
  // anyone with the URL, so a predictable username+timestamp name would let a private
  // community's photos be guessed/enumerated.
  const fname = `${safe}-${Date.now()}-${crypto.randomBytes(16).toString('hex')}.${ext}`;
...
function addAudit(communityId, actor, action, target = '', metadata = {}) {
  ...
  // keep only the newest AUDIT_MAX_PER_COMMUNITY for this community (push order is
  // chronological). Trimming globally let a member of one community evict another
  // community's moderation record just by repeatedly editing their own photos.
  const mine = auditEvents.filter(e => e.communityId === communityId);
  if (mine.length > AUDIT_MAX_PER_COMMUNITY) {
    const drop = new Set(mine.slice(0, mine.length - AUDIT_MAX_PER_COMMUNITY).map(e => e.id));
    for (let i = auditEvents.length - 1; i >= 0; i--) if (drop.has(auditEvents[i].id)) auditEvents.splice(i, 1);
  }
...
function addNotification(toUsername, communityId, type, actor, postId, extra = {}) {
  ...
  // don't leak community activity to someone who is no longer a member (removed or
  // banned): their bearer token may still be valid and their old posts may still
  // exist, but they must not keep receiving readable notifications about it.
  if (communityId && !isCommunityMember(findCommunity(communityId), toUsername)) return;
  notifications.push({
    id: crypto.randomBytes(8).toString('hex'),
    to: toUsername,
    communityId,
...
function memberList(c) {
  // one pass over this community's posts -> per-user photo counts, so we don't
  // re-scan the entire global posts array once per member (was O(members x posts)).
  const counts = Object.create(null);
  for (const p of scopedPosts(c.id)) counts[p.username] = (counts[p.username] || 0) + 1;
  return Object.entries(communityMembers(c))
    .map(([username, role]) => users[username] ? { ...publicProfile(users[username], c.id, counts[username] || 0), role } : null)
    .filter(Boolean)
    .sort((a, b) => {
...
async function deleteCommunityAndContent(communityId) {
  const community = communities.find(cc => cc.id === communityId);
  if (community && community.cover) await deleteImage(community.cover);   // don't orphan the cover image
  ...
  for (let i = auditEvents.length - 1; i >= 0; i--) if (auditEvents[i].communityId === communityId) auditEvents.splice(i, 1);
...
async function deleteUserAccount(username) {
  ...
  // remove the account's own avatar/cover images so they aren't orphaned in storage
  // (owned-community covers and the user's post images are cleaned up below).
  const acct = users[name];
  if (acct.avatar) await deleteImage(acct.avatar);
  if (acct.cover) await deleteImage(acct.cover);
  ...
  for (let i = auditEvents.length - 1; i >= 0; i--) if (auditEvents[i].actor === name) auditEvents.splice(i, 1);
  for (const tok of Object.keys(sessions)) {
    const s = sessions[tok];
    const sn = typeof s === 'string' ? s : s && s.username;
  ...
  ['users.json', 'posts.json', 'albums.json', 'communities.json', 'invites.json', 'notifications.json', 'sessions.json', 'audit_events.json']
