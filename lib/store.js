...
/* Data layer: the in-memory collections every other module shares, plus a
   ...
   dev) or against Supabase Postgres (production).
   ...
   Two drivers, chosen by STORE_DRIVER:
     ...
     - 'supabase' : stores each collection as one JSONB blob in a Supabase
                   ...
                   fetch. No local filesystem, so state survives a restart or a
                   redeploy on a host with no persistent disk.
...
function fillInPlace(target, src) {
  if (Array.isArray(target)) {
    target.length = 0;
    // for-of, NOT push(...src): spreading passes one argument per element, so a
    // collection past the engine's argument limit (~125k) throws RangeError here on
    // every cold start - which would brick the app permanently, since the data that
    // triggers it is exactly the data being loaded.
    if (Array.isArray(src)) for (const v of src) target.push(v);
  } else {
...
async function persistBatch(batch) {
  ...
  for (const [file, obj] of batch) {
    ...
    // Re-queue from the LIVE collection, not from this batch's stale snapshot. The
    // snapshot was taken before the write and can be older than changes committed
    // while it was in flight, so retrying it would roll those back (silent data
    // loss). The in-memory collection is always the newest truth. Never clobber a
    // newer snapshot a concurrent saveJSON already queued for this file.
    catch (e) {
      if (!dirty.has(file)) dirty.set(file, structuredClone(collections[file]));
...
// how many collections still have unpersisted changes (for shutdown drain / health)
function pendingWrites() { return dirty.size; }

module.exports = {
  ROOT, DATA_DIR, ASSETS_DIR, UPLOAD_DIR,
  init, flush, saveJSON, pendingWrites,
