...
// Text assets are static for the life of the process, so compress each once and
// cache the bytes in memory (keyed by file + mtime), then serve the same buffer to
// every visitor. This ships the ~1.2MB Three.js / 226KB main.js / 108KB CSS ~4x
// smaller without re-compressing per request. Already-compressed formats
// (woff2/jpg/png/webp) are skipped - gzipping them wastes CPU for ~no gain.
const COMPRESSIBLE = new Set(['.js', '.css', '.html', '.svg', '.json']);
const encodeCache = new Map();
function compressedVariant(f, mtimeMs, data, enc) {
  const key = `${f}:${mtimeMs}:${enc}`;
  let buf = encodeCache.get(key);
  if (buf) return buf;
  buf = enc === 'br'
    ? zlib.brotliCompressSync(data, { params: { [zlib.constants.BROTLI_PARAM_QUALITY]: 5 } })
    : zlib.gzipSync(data, { level: 6 });
  if (encodeCache.size > 128) encodeCache.clear();   // bound; re-fills lazily after a deploy
  encodeCache.set(key, buf);
  return buf;
}
...
function serveStatic(req, res, pathname) {
  ...
  fs.stat(f, (statErr, st) => {
    ...
    const headers = securityHeaders({
      ...
      // caches must key on the encoding so a gzip/br copy isn't served to a client
      // that sent a different Accept-Encoding
    ...
    fs.readFile(f, (err, data) => {
      ...
      const enc = compressible ? pickEncoding(req) : '';
      if (enc) {
        const body = compressedVariant(f, Math.round(st.mtimeMs), data, enc);
