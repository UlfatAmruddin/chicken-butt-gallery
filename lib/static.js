...
function securityHeaders(extra = {}) {
  const headers = {
    ...
    'Content-Security-Policy': [
      "default-src 'self'",
      // Three.js and GSAP are now self-hosted (js/vendor), so no inline scripts
      // and no third-party script origin are needed - the strictest sensible CSP.
      "script-src 'self'",
      // style-src must keep 'unsafe-inline': GSAP writes inline styles and the
      // markup uses a few style="" attributes. Google Fonts supplies the CSS.
  ...
  // HSTS is meaningful only over HTTPS (browsers ignore it on plain HTTP), so it
  // is safe to always send and takes effect once the TLS proxy is in front.
  if (config.ENABLE_HSTS && !('Strict-Transport-Security' in headers)) {
...
function serveStatic(req, res, pathname) {
  ...
  // Uploaded assets and vendored libraries are effectively immutable (timestamped
  // / version-pinned filenames) so they can cache hard; the app shell stays
  // no-store so deploys take effect immediately.
  const cacheable = routePath.startsWith('/assets/') || routePath.startsWith('/js/vendor/');
  const cacheControl = cacheable ? 'public, max-age=31536000, immutable' : 'no-store';
