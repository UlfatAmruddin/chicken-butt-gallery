...
async function uploadDataUrl(dataUrl, subdir, basename, maxBytes = 12 * 1024 * 1024) {
  ...
  if (dimensionsTooLarge(buf, m[1])) return { error: `Image resolution too large (${MAX_MEGAPIXELS}MP max).` };
...
module.exports = { isConfigured, isSupabaseUrl, uploadDataUrl, deleteByUrl, sniff, dimensionsTooLarge };
